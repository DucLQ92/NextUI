...
static const char *builtin_vi[][2] = {
    ...
    {"Show battery level as percent in the status pill", "Hiển thị con số % pin chính xác trên thanh trạng thái"},
    ...
    {"If enabled, used the emulator background image. Otherwise uses the default.", "Dùng ảnh nền riêng của từng hệ máy thay vì ảnh nền mặc định."},
    ...
    {"White point correction green channel gain (0 to 200)", "Tăng giảm kênh màu xanh lá khi hiệu chỉnh (0 đến 200)"},
    {"Blue gain", "Kênh màu xanh dương (Blue Gain)"},
    {"White point correction blue channel gain (0 to 200)", "Tăng giảm kênh màu xanh dương khi hiệu chỉnh (0 đến 200)"},
    ...
    {"Period of inactivity before screen turns off (0-600s)", "Thời gian không hoạt động trước khi tắt màn hình (0-600 giây)"},
    {"Suspend timeout", "Thời gian ngủ"},
    {"Time before device goes to sleep after screen is off (5-600s)", "Thời gian trước khi máy vào chế độ ngủ sau khi tắt màn hình (5-600 giây)"},
    {"Haptic feedback", "Rung phản hồi"},
    {"Enable or disable haptic feedback on certain actions in the OS", "Bật hoặc tắt rung phản hồi cho các thao tác trong hệ thống"},
    ...
    {"Select the fan speed percentage (Quiet/Normal/Performance or 0-100%)", "Chọn mức tốc độ quạt (Yên tĩnh / Bình thường / Hiệu năng hoặc 0-100%)"},
    {"Reset to defaults", "Khôi phục mặc định"},
    {"Resets all options in this menu to their default values.", "Đặt lại tất cả tùy chọn trong menu này về giá trị mặc định ban đầu."},
    ...
    {"DotClean", "Dọn tệp rác Mac (DotClean)"},
    {"Dot Clean", "Dọn tệp rác Mac (DotClean)"},
    {"Dot_Clean", "Dọn tệp rác Mac (DotClean)"},
    {"dot_clean", "Dọn tệp rác Mac (DotClean)"},
    {"Updater", "Cập nhật"},
